...
tf.app.flags.DEFINE_string('data_dir', 'cnn_roi_data',
                           """Path to the CIFAR-10 data directory.""")
tf.app.flags.DEFINE_boolean('use_fp16', False,
                            """Train the model using fp16.""")

# Global constants describing the CIFAR-10 data set.
IMAGE_SIZE = cnn_roi_input.IMAGE_SIZE
NUM_CLASSES = cnn_roi_input.NUM_CLASSES
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = cnn_roi_input.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = cnn_roi_input.NUM_EXAMPLES_PER_EPOCH_FOR_EVAL
...
def distorted_inputs():
  ...
  data_dir = os.path.join(FLAGS.data_dir, 'cnn_roi_data')
  images, labels = cnn_roi_input.distorted_inputs(data_dir=data_dir,
...
def inputs(eval_data):
  ...
  if not FLAGS.data_dir:
    raise ValueError('Please supply a data_dir')
  data_dir = os.path.join(FLAGS.data_dir, 'cnn_roi_data')
  images, labels = cnn_roi_input.inputs(eval_data=eval_data,
                                        data_dir=data_dir,
                                        batch_size=FLAGS.batch_size)
  if FLAGS.use_fp16:
    images = tf.cast(images, tf.float16)
...
def inference(images, isTraining):
  ...
  with tf.variable_scope('local8') as scope:
    weights = _variable_with_weight_decay('weights', shape=[2048, 512],
                                          stddev=0.04, wd=0.004)
    biases = _variable_on_cpu('biases', [512], tf.constant_initializer(0.1))
    local8 = tf.nn.relu(tf.matmul(local7, weights) + biases, name=scope.name)
    _activation_summary(local8)

  dropout = tf.layers.dropout(inputs=local8, rate=0.5, training=isTraining)
  ...
  with tf.variable_scope('softmax_linear') as scope:
    ...
    softmax_linear = tf.add(tf.matmul(dropout, weights), biases, name=scope.name)
...
def maybe_download_and_extract():
  ...
  extracted_dir_path = os.path.join(dest_directory, 'cnn_roi_data')
